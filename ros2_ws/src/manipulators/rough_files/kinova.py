...
@dataclass
class RobotFeedback:
    """Raw feedback from robot."""
    positions_deg: np.ndarray  # 7 values
    velocities_deg: np.ndarray  # 7 values
    torques_measured: np.ndarray  # 7 values
    timestamp: float
...
class KinovaHardware:
    ...
    def _init_command_structure(self) -> None:
        ...
        # Setup interconnect for gripper control in low-level mode
        self._base_command.interconnect.command_id.identifier = 0
        gripper_motor = self._base_command.interconnect.gripper_command.motor_cmd.add()
        gripper_motor.motor_id = 0
        gripper_motor.position = 0.0
        gripper_motor.velocity = 100.0
    ...
    def send_torques(
        ...
        Args:
            ...
            gripper_position: Gripper target, 0.0=open, 1.0=closed
        ...
        try:
            ...
            # Update gripper via interconnect (0-1 → 0-100%)
            self._base_command.interconnect.command_id.identifier = self._base_command.frame_id
            self._base_command.interconnect.gripper_command.motor_cmd[0].position = gripper_position * 100.0
    ...
    def read_gripper_position(self) -> float:
        ...
        try:
            ...
            measure = self.base.GetMeasuredGripperMovement(gripper_request)
            if measure.finger:
    ...
    def send_gripper_command(self, value: float) -> None:
        ...
        Args:
            value: Gripper position, 0.0 = fully open, 1.0 = fully closed.
        """
        if not self.base:
            return
